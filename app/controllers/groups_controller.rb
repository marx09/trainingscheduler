...
class GroupsController < ApplicationController
  ...
  def destroy
    ...
    if can?(:manage, group)
      group.destroy
      redirect_to '/groups', success: 'Group deleted.
        All members are available for adding to another group.'
    end
  end
  ...
  def add_user
    ...
    respond_to do |format|
      if can?(:manage, @user) && @group.save
        ...
        format.js { render action: 'add_user', status: :accepted }
      else
        format.js { render json: @group.errors, status: :unprocessable_entity }
      end
    end
  ...
  def remove_user
    ...
    respond_to do |format|
      if can?(:manage, @user) && @user.save
        ...
        format.js { render action: 'remove_user', status: :accepted }
      else
